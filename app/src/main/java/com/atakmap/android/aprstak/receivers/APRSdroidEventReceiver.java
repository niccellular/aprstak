								...
								if (AprsUtility.usePSK) {
									...
									try {
										...
										try {
											...
											InputSource is = new InputSource(new ByteArrayInputStream(cotAsBytes));
											SAXSource exiSource = new EXISource(exiFactory);
											exiSource.setInputSource(is);
											TransformerFactory tf = TransformerFactory.newInstance();
											Transformer transformer = tf.newTransformer();
											transformer.transform(exiSource, result);
											cotEvent = CotEvent.parse(writer.toString());

											if (cotEvent != null && cotEvent.isValid()) {
												CotMapComponent.getInternalDispatcher().dispatch(cotEvent);
											} else {
												Log.d(TAG, "Bad CotEvent: " + cotEvent);
											}
										} catch (Throwable e) {
											e.printStackTrace();
										}
									...
									try {
										...
										StringWriter writer = new StringWriter();
										Result result = new StreamResult(writer);
										InputSource is = new InputSource(new ByteArrayInputStream(payload));
										SAXSource exiSource = new EXISource(exiFactory);
										exiSource.setInputSource(is);
										TransformerFactory tf = TransformerFactory.newInstance();
										Transformer transformer = tf.newTransformer();
